...
class VOCXMLProcessor:
    ...
    def get_all_categories_and_images_batch(self, xml_dir: str, recursive: bool = False) -> Dict[str, Dict[str, List[str]]]:
        """
        ...
        # 获取目录下所有XML文件
        xml_files = get_files(xml_dir, '.xml', recursive=recursive)
        all_data = {}
...
class YOLOTXTProcessor:
    ...
    def __init__(self, verbose: bool = False):
        ...
        """
        self.verbose = verbose
        self.logger = set_logging("YOLOTXTProcessor", verbose=verbose)

    def update_categories(self, txt_path: str, all_categories: List[str], source_categories: List[str], target_categories: List[str]) -> int:
        """
        更新 TXT 文件中的类别序号（YOLO 格式）

        Args:
            txt_path: TXT 标注文件的完整路径或包含 TXT 文件的目录路径
            all_categories: 所有原始类别名称列表（用于建立 ID 映射）
            source_categories: 需要被替换的原始类别名称列表
            target_categories: 替换后的目标类别名称列表
        ...
        Raises:
            ValueError: 当 txt_path 不是 TXT 文件或目录时
            ValueError: 当源类别和目标类别列表长度不匹配时
        ...
        # 建立类别名 -> 类别ID 映射
        category_to_id = {cat: idx for idx, cat in enumerate(all_categories)}
        ...
        if os.path.isfile(txt_path) and txt_path.endswith(".txt"):
            return self._update_single_file_categories(txt_path, source_to_target_id)
        elif os.path.isdir(txt_path):
            txt_files = get_files(txt_path, ".txt")
            total_updated = 0
            for txt_file in tqdm(txt_files, desc=f"更新 TXT 类别: {txt_path}"):
                try:
            ...
            raise ValueError(f"txt_path 必须是 TXT 文件或目录: {txt_path}")
